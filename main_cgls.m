...
%% CGLS
X = cgls_acc(G,y_noisy);

%% PLOT 1: "BEST" CGLS SOLUTIONS
figure('Name','"Best" CGLS Solutions');
plot(x_true,'-k','DisplayName','x_{true}');
hold on;
for j = 5:7
    plot(X(1:N/2,j),'-.','DisplayName',strcat("k =", num2str(j)));
end
title('\textbf{CGLS Solutions} $$\mathbf{x}_k$$','Interpreter','latex');
...
semilogx(norm_rk(1:N/20),norm_xk(1:N/20),'.-','MarkerSize',12);
hold on;
